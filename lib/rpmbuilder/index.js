...
function setupTempDir(tmpDir) {
  ...
  if (fsx.existsSync(tmpDir)) {
    logger.info(`Removing old temporary directory at ${tmpDir}`);
    fsx.removeSync(tmpDir);
    console.log(`After removing tmpDir listing: ${execSync('ls -lart /tmp').toString()}`)
  }

  // Create RPM folder structure
  logger.info(`Creating RPM directory structure at: ${tmpDir}`);
...
function prepareFiles(files, excludeFiles, buildRoot) {
  ...
  _.forEach(files, function(file) {
    ...
    _.forEach(actualSrc, function(srcFile) {
      ...
      fsx.moveSync(srcFile, path.join(buildRoot, dest));
    });
  });
    ...
    if (stdout) {
      ...
      if (rpm && rpm.length > 0) {
        ...
        if (rpmDest) {
          ...
          logger.info(`Moving RPM ${rpm[0]} package to: ${rpmDestination}`);
          //fsx.copySync(rpm[0], rpmDestination);
          console.log(`Before moving rpm listing: ${execSync('ls -lart /tmp').toString()}`)
          fsx.moveSync(rpm[0], rpmDestination);
          console.log(`After moving rpm listing: ${execSync('ls -lart /tmp').toString()}`)
        }
...
function rpm(options, cb) {
  ...
  logger = options.verbose ? require('./Logger') : function() {
    return;
  };
  ...
  logger.info(`SPEC file created: ${specFile}`);
  ...
  buildRpm(buildRoot, specFile, options.rpmDest, options.execOpts, function(err, rpm) {
    ...
    // Remove temp folder
    if (!options.keepTemp) {
      logger.info(`Removing RPM directory structure at: ${tmpDir}`);
      fsx.removeSync(tmpDir);
    }
